...
def _kvarn_mla_decode_kernel(
    ...
    program; loops the sequence's tokens via its block_table row, dequants the
    packed latent in-kernel (4-bit + per-token scale/zp), uses it as K and V,
    adds fp16 RoPE, online softmax. Validated form (proto cos 1.0)."""
    ...
    for s in range(0, seq_len):
        phys = tl.load(BlockTable + bcur * stride_btb + s // PAGE)
        base = (phys * PAGE + s % PAGE) * REC
...
class TritonMLAImpl(MLACommonImpl[MLACommonMetadata]):
    ...
    def do_kv_cache_update(self, kv_c_normed, k_pe, kv_cache, slot_mapping,
        ...
        rec = torch.empty(T, REC, dtype=torch.uint8, device=kv_cache.device)
        rec[:, :NB] = packed
        rec[:, NB:NB + 2] = scale.squeeze(1).to(torch.float16).view(torch.uint8).reshape(T, 2)
        rec[:, NB + 2:NB + 4] = lo.squeeze(1).to(torch.float16).view(torch.uint8).reshape(T, 2)
        rec[:, NB + 4:NB + 4 + RP * 2] = rope.reshape(T, RP).view(torch.uint8).reshape(T, RP * 2)
    ...
    def forward_mqa(
        ...
        if self._is_kvarn_mla:
            ...
            NB = (self.kv_lora_rank * self._kvarn_bits) // 8
            REC = kv_c_and_k_pe_cache.shape[-1]
            ...
            _kvarn_mla_decode_kernel[(B, q_num_heads)](
                ...
                SCALE_OFF=NB, ZP_OFF=NB + 2, ROPE_OFF=NB + 4, PAGE=PAGE,
            )
            return o, lse
